...
const TimeTracker: React.FC = () => {
  ...
  const updateHours = (index: number, newHours: number) => {
    ...
    const totalUsedHoursBeforeUpdate = updatedAreas.reduce(
      (sum, area) => sum + area.hours,
      0
    );
    ...
    if (
      newHours >= 0 &&
      newHours <= remainingHoursBeforeUpdate + updatedAreas[index].hours
    ) {
      updatedAreas[index].hours = newHours;
      const totalUsedHours = updatedAreas.reduce(
        (sum, area) => sum + area.hours,
        0
      );
  ...
  return (
    <div className="flex justify-center items-center space-x-8 ">
      ...
      <DonutChart areas={areas} />{" "}
      {/* Pass the areas as a prop to DonutChart */}
    </div>
  );
};
